...
class TestDataCleanup {
  ...
  async getAllForms() {
    const response = await this.makeRequest('GET', '/api/forms', null, {
  ...
  isTestForm(form) {
    ...
    const isTest =
      testPatterns.some((pattern) => pattern.test(form.refKey)) ||
  ...
  async deleteTestForm(refKey) {
    ...
    try {
      ...
      const response = await this.makeRequest(
        'DELETE',
        `/api/forms?refKey=${encodedRefKey}`,
        null,
        {
          'x-admin-key': this.config.adminApiKey,
        }
      ...
      if (response.status === 200 && response.data.ok) {
        ...
        console.log(
          `⚠️  Could not delete form ${refKey}: ${response.data.error?.message || 'Unknown error'}`
        )
        return false
      }
    } catch (error) {
  ...
  async cleanupTestSubmissions() {
    ...
    for (const sheetUrl of testSheets) {
      ...
      try {
        const response = await this.makeRequest(
          'DELETE',
          `/api/responses?sheetUrl=${encodeURIComponent(sheetUrl)}`,
          null,
          {
            'x-admin-key': this.config.adminApiKey,
          }
        ...
        if (response.status === 200 && response.data.ok) {
          ...
          console.log(
            `   ⚠️  Could not clean sheet responses: ${response.data.error?.message || 'Unknown error'}`
          )
        }
      } catch (error) {
  ...
  async cleanupTestScreenshots() {
    ...
    const screenshotDirs = ['./tests/screenshots', './tests/results']
    ...
    for (const dir of screenshotDirs) {
      try {
        ...
        const testFiles = files.filter(
          (file) => file.match(/\.(png|jpg|jpeg|html|json)$/) && file.match(/\d{13}/) // contains timestamp
        )
