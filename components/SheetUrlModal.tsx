...
• The Apps Script has been granted access to this spreadsheet`
            ...
            baseErrorMsg.includes('permission') ||
            baseErrorMsg.includes('access') ||
            baseErrorMsg.includes('Unable to access spreadsheet structure')
          ) {
...
3. Try redeploying the Apps Script project`
  ...
  function checkStructureMismatch(sheet: SheetMeta['sheets'][0] | undefined): {
    hasContent: boolean
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div>
              <label className="block text-sm font-medium">Select Target Sheet *</label>
              <span className="text-xs text-gray-500">
                New/Rename sheets may take 2-5 minutes to appear
              </span>
            </div>
            {sheetsMeta && (
              <Button
                    ...
                    <div className="space-y-2">
                      <div className="rounded-md bg-gray-50 p-3 text-sm">
                        ...
                        <p className="mt-1 text-gray-600">
                          {hasContent
                            ? `This sheet has ${sheet.rows} rows of data`
                            : 'This sheet is empty'}
                        </p>
                      </div>
                      ...
                      {hasContent && !structureMatches && (
                        <div className="rounded-md border border-yellow-200 bg-yellow-50 p-3 text-sm">
                          ...
                          <p className="mt-1 text-yellow-700">
                            The existing sheet headers don't match your form fields. Headers will be
                            automatically updated when you save the form.
                          </p>
                          <details className="mt-2">
                            ...
                            <div className="mt-2 text-xs text-yellow-600">
                              <p>
                                Current headers: {sheet.headers?.slice(0, 5).join(', ')}
                                {(sheet.headers?.length || 0) > 5 ? '...' : ''}
                              </p>
                              <p className="mt-1">
                                Expected headers: timestamp, refKey, ip, userAgent
                                {currentFields.length > 0
                                  ? ', ' + currentFields.map((f) => f.key).join(', ')
                                  : ''}
                              </p>
                            </div>
                          </details>
